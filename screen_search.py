...
def start_game(ahk, handle):
    ...
    image_path = f'./images/{x2 - x1}_{y2 - y1}/start_game.png'
...
def open_rift(ahk, handle, rift_type):
    x1, y1, x2, y2 = win32gui.GetWindowRect(handle)
    image_path = f'./images/{x2 - x1}_{y2 - y1}/obelisk.png'
...
def gamble(ahk, handle, item):
    x1, y1, x2, y2 = win32gui.GetWindowRect(handle)
    image_path = f'./images/{x2 - x1}_{y2 - y1}/kadala.png'
    x1, y1 = transform_coordinates(handle, 220, 30)
    x2, y2 = transform_coordinates(handle, 300, 100)
    found = ahk.image_search(image_path, (x1, y1), (x2, y2), 30)
    if found:
...
def accept_gr(ahk, handle):
    x1, y1, x2, y2 = win32gui.GetWindowRect(handle)
    image_path = f'./images/{x2 - x1}_{y2 - y1}/keystone.png'
...
def upgrade_gem(ahk, handle):
    x1, y1, x2, y2 = win32gui.GetWindowRect(handle)
    image_paths = [
        f'./images/{x2 - x1}_{y2 - y1}/urhsi_upgrade_{i}.png' for i in range(1, 6)
    ]
...
wd = sys._MEIPASS
image_path = os.path.join(wd, f'./images/start_game_{x2 - x1}_{y2 - y1}.png')
"""
